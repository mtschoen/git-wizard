...
- **MFTLib** - Used on Windows for fast .git discovery via NTFS MFT parsing. When already elevated, `GitWizardApi.TryFindAllRepositoriesUsingMftAsync` scans in-process; when not, it takes the cold scan through MFTLib's journal broker (`JournalBrokerClient`, one UAC prompt) and pulls repository roots from the returned `ScanRecord`s - no temp-file handoff. GitWizard requests `BrokerScanProfile.DirectoryIndexWithGitPointers`, retaining directories plus `.git` files rather than packing every file path into the broker's single MMF; the full C: inventory exceeds the managed 2 GiB payload limit on this machine.
...
- **Journal watch (`--watch` / UI Live mode, Windows only)** - Auto-detects changes in tracked repositories via MFTLib's elevated journal broker, one UAC prompt for the whole run. The pipeline lives in `GitWizard/Watch/` behind `IVolumeChangeSource` (platform seam): `UsnVolumeChangeSource` is the Windows adapter over MFTLib's `VolumeBroker` (`JournalBrokerClient`/`JournalBrokerHost`), resolving live/catch-up USN entries to paths via a per-drive record-number index seeded from the cold scan. `RepositoryWatchService` is the platform-agnostic layer on top: maps volume change batches to repository roots via `GitWizard.RepositoryChangeFilter.Classify` (path-based; Changed/Created/Deleted, deduped and debounced per window) and surfaces per-drive scan failures via `ScanErrors`. `Program.cs` dispatches MFTLib's elevated `--broker` child mode via `ElevatedEntryPoint.TryHandle` before any other startup work (see `Program.Watch.cs`'s `TryHandleElevatedBrokerEntry` seam); `--watch` constructs a `UsnVolumeChangeSource` + `RepositoryWatchService` and prints `changed: <root>` / `created: <root>` / `deleted: <root>` per event plus `scan error on <drive>: <message>` for drives that failed to arm. Rename correlation is deliberately left to consumers and is not printed by the CLI. GitWizardUI's opt-in Live button runs the same service through `LiveWatchController`, shows an amber `Starting Live...` state while the broker arms, turns green only after `RepositoryWatchService.Started`, applies events on the UI thread, and correlates delete/create pairs by remote URL to preserve renames.
