...
- **Journal watch (`--watch`, Windows only, CLI)** - Auto-detects changes in tracked repositories via MFTLib's elevated journal broker, one UAC prompt for the whole run. The pipeline lives in `GitWizard/Watch/` behind `IVolumeChangeSource` (platform seam): `UsnVolumeChangeSource` is the Windows adapter over MFTLib's `VolumeBroker` (`JournalBrokerClient`/`JournalBrokerHost`), resolving live/catch-up USN entries to paths via a per-drive record-number index seeded from the cold scan. `RepositoryWatchService` is the platform-agnostic layer on top: maps volume change batches to repository roots via `GitWizard.RepositoryChangeFilter.Classify` (path-based; Changed/Created/Deleted, deduped and debounced per window) and surfaces per-drive scan failures via `ScanErrors`. `Program.cs` dispatches MFTLib's elevated `--broker` child mode via `ElevatedEntryPoint.TryHandle` before any other startup work (see `Program.Watch.cs`'s `TryHandleElevatedBrokerEntry` seam); `--watch` itself constructs a `UsnVolumeChangeSource` + `RepositoryWatchService` and prints `changed: <root>` / `created: <root>` / `deleted: <root>` per event plus `scan error on <drive>: <message>` for drives that failed to arm. Rename correlation is deliberately left to consumers (e.g. a future UI) and is not printed by the CLI. Not wired into GitWizardUI yet.
