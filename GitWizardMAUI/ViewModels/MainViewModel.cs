...
namespace GitWizardMAUI.ViewModels;

public class MainViewModel : INotifyPropertyChanged, IUpdateHandler
{
    private readonly ConcurrentDictionary<string, RepositoryNodeViewModel> _repositoryMap = new();
    private readonly ConcurrentQueue<RepositoryUICommand> _uiCommands = new();
    private readonly Stopwatch _stopwatch = new();

    private string _headerText = "GitWizard";
    ...
    private void OpenInExplorer(RepositoryNodeViewModel? node)
    {
    ...
    private void OpenInFork(RepositoryNodeViewModel? node)
    {
        if (node == null || string.IsNullOrEmpty(node.WorkingDirectory))
            return;
    ...
    private void ProcessUICommand(RepositoryUICommand command)
    {
        switch (command.Type)
        {
    ...
    private void AddSubmodule(Repository parent, Repository submodule)
    {
    ...
    private void AddUninitializedSubmodule(Repository parent, string submodulePath)
    {
        // For now, we'll skip uninitialized submodules in the tree view
        // Could be added as a special node type in the future
    }

    private void UpdateCompletedRepository(Repository repository)
    {
        var path = repository.WorkingDirectory;
    ...
    private enum RepositoryUICommandType
    {
        RepositoryCreated,
        SubmoduleCreated,
        WorktreeCreated,
    ...
    private struct RepositoryUICommand
    {
