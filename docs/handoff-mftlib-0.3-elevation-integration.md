...
## Status: Parts B–E done (validated, `dotnet test` is UAC-free). Pending: MFTLib 0.3.0 publish + ref flip for the real PR.
...
The code work (Parts B–E) is complete and validated UAC-free on this branch. What remains is gated on the publish:
...
4. **Before the real PR:** once 0.3.0 is published, flip `GitWizard/GitWizard.csproj` back to `<PackageReference Include="MFTLib" Version="0.3.0" />` (the `BlockLocalMFTLibOnPublish` target enforces this on `dotnet publish`), drop `-NoBuild` from coverage runs (plain `dotnet` works again), re-verify, then open the git-wizard PR.
